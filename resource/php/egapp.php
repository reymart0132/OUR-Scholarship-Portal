...
if(!empty($_POST)) {
  ...
  session_destroy();
  header("location:../../egapplication?status=captchaError");
  die();
  }
  ...
  $transnumber = uniqid('ceuEGA');

  $email = $_POST['email'];

  if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
    echo "email invalid";
    header("location:../../egapplication?status=tamper");
    die();
    }else{
    echo "email valid";
  }
  ...
  $required = array( 'lastname', 'firstname', 'formerschool', 'awardtitle', 'ylevel', 'Section', 'addressfs', 'dob', 'birthplace', 'haddress', 'caddress', 'contactnumber','email', 'fathername', 'foccupation', 'femployer', 'fincome', 'mothername', 'moccupation', 'memployer', 'mincome', 'Course','genave');
  $error = false;
  ...
  if(datevalidationEGA($studentnumber) == false){
    header("location:../../egapplication?status=MultipleTrans");
    die();
  }
  ...
  $reds=0;
  require_once('eguploadfile.php');
  if($reds ==1){
    header("location:../../esapplication?status=upload1");
    die();
  }
  $transaction = new egainsert($studentnumber, $lastname, $firstname, $middlename, $formerschool, $college, $awardtitle, $year, $section, $addressfs, $dateapplied, $dob, $birthplace, $haddress, $caddress, $contacts,$email, $fathername, $foccupation, $femployer, $fincome, $mothername, $moccupation, $memployer, $mincome, $course,$genave, $awards1, $school1, $awards2, $school2, $awards3, $school3, $org1, $position1, $org2, $position2, $org3, $position3, $org4, $position4,$transnumber,$file);
  $transaction->insertEGApplicationData();
  $transaction->insertEGApplication();
  header("location:../../rof64?tn=".urlencode(base64_encode($transnumber)));
}
 ?>
