...
if(!empty($_POST)) {
  ...
  session_destroy();
  header("location:../../usapplication?status=captchaError");
  die();
  }
  ...
  if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
    echo "email invalid";
    header("location:../../usapplication?status=tamper");
    die();
    }else{
    echo "email valid";
  }
...
if ($cgrant == "University Academic Scholarship") {
    if($GWA <= 1.204 ){
      ...
      header("location:../../usapplication?status=notElig");
      die();
    }else{
  ...
  if(datevalidationUSA($studentnumber) == false){
    header("location:../../usapplication?status=MultipleTrans");
    die();
  }
  ...
  if($reds ==1){
    header("location:../../usapplication?status=upload1");
    die();
  }
  ...
  header("location:../../rof15?yl=$ylevel&st=$Section&discount=$discount&duration=$duration&tn=".urlencode(base64_encode($transnumber)));
}
 ?>
